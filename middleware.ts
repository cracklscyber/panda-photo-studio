...
export async function middleware(req: NextRequest) {
  ...
  // HEAD the Supabase public URL for index.html. If present, this site is custom
  // and should be served from the /custom-site route handler.
  ...
  const rewritePath = rest.length > 0
    ? `/custom-site/${slug}/${rest.join('/')}`
    : `/custom-site/${slug}`
